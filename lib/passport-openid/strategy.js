  ...
  if (typeof options.attributes === 'object') {
    for (var attr in options.attributes) {
      if (!attributes.hasOwnProperty(attr)) {
        attributes[attr] = options.attributes[attr];
      }
    }
  }
 ...
 * @api protected
 */
Strategy.prototype.authenticate = function(req, queryString) {
  ...
  } else if (req.body && req.body['openid.mode']) {
    var hasQs = /\?.+/.test(req.url)

    oidRequest = {
      mode: req.body['openid.mode'],
      returnUrl: req.url + (hasQs ? '&' : '?') + qs.stringify(req.body)
    };
  }
  ...
  if (oidRequest) {
    ...
    if (oidRequest.mode === 'cancel') { return this.fail({ message: 'OpenID authentication canceled' }); }
    
    var self = this;
    this._relyingParty.verifyAssertion(oidRequest.returnUrl, function(err, result) {
      ...
      var axProfile = self._parseAx(result);
      var pape = self._parsePAPEExt(result);
      var oauth = self._parseOAuthExt(result);

      // merge profiles
      for (var attr in axProfile) {
        profile[attr] = axProfile[attr];
      }
...
/**
 * Parse Attribute Exchange profile from OpenID response.
 */
Strategy.prototype._parseAx = function(params) {
  var profile = {};
  var regexpAx = /http:\/\/axschema\.org/;
  var regexpCznic = /http:\/\/specs\.nic\.cz/;

  for (var attr in params) {
    if (attr.match(regexpAx) || attr.match(regexpCznic)) {
